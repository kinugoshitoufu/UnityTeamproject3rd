...
/// </summary>
[System.Serializable]
public class TakedaPlayerAction
{
...
/// </summary>
public class TakedaPlayerScript : MonoBehaviour
{
    // ========== 様々な機能に必要な変数 ==========
    private bool flicflag = false;
    public static TakedaPlayerScript instance;
    private Animator animator;
    private bool _evilStareStop = false;//--------------------------------------------------------←蛇睨み追加分
    ...
    private List<TakedaPlayerAction> recordedActions = new List<TakedaPlayerAction>();
    ...
    void Update()
    {
        ...
        if (isRecording&&EvilStareStop==false)  //--------------------------------------------------------←蛇睨み追加分
        {
            RecordPlayerInput();
        }
        ...
        // 速度が0で、かつ入力もない状態
        if (rb.linearVelocity == Vector2.zero && Mathf.Abs(horizontal) == 0.0f&&EvilStareStop==false)//--------------------------------------------------------←蛇睨み追加分
        {
    ...
    void RecordPlayerInput()
    {
        ...
        // ========== 現在の状態を記録 ==========
        TakedaPlayerAction action = new TakedaPlayerAction
        {
    ...
    void StopRecordingAndSpawnClone()
    {
        ...
        // クローンのコントローラーを取得
        TakedaCloneController cloneController = clone.GetComponent<TakedaCloneController>();

        if (cloneController != null)
        {
            // クローンに記録データを渡す（新しいListを作成してコピー）
            cloneController.SetRecordedActions(new List<TakedaPlayerAction>(recordedActions));
        }
        else
        {
