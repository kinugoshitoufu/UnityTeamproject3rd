...
/// </summary>
public class TakedaCloneController : MonoBehaviour
{
    // ========== 様々な機能に必要な変数 ==========
    private bool flicflag = false;
    private float DefCloneScale;
    private float CloneScale;
    public static TakedaCloneController instance;

    // ========== 記録データ関連 ==========
    [Tooltip("再生する行動データのリスト")]
    public List<TakedaPlayerAction> recordedActions;
    ...
    public void SetRecordedActions(List<TakedaPlayerAction> actions)
    {
        // 記録データを受け取る
    ...
    void ReplayActions()
    {
        ...
        {
            // 現在のアクションを取得
            TakedaPlayerAction currentAction = recordedActions[currentActionIndex];
            ...
            if (useInterpolation && currentActionIndex < recordedActions.Count - 1)
            {
                ...
                TakedaPlayerAction nextAction = recordedActions[currentActionIndex + 1];
